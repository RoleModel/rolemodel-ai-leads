...
import { Download01Icon, Calendar03Icon, RefreshIcon, FilterIcon } from "@hugeicons-pro/core-stroke-standard"
...
export default function LeadsPage() {
  ...
  async function handleExport() {
    try {
      const res = await fetch(`/api/leads/export?startDate=${dateRange.start}&endDate=${dateRange.end}`)
        ...
        <main style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{
                ...
                <HugeiconsIcon icon={FilterIcon} size={16} style={{ color: 'var(--op-color-neutral-on-plus-max)' }} />
                <span style={{ fontSize: 'var(--op-font-small)', fontWeight: 600 }}>Filters</span>
              </div>
              <div style={{
                display: 'flex',
                alignItems: 'center',
                ...
                <span style={{ fontSize: 'var(--op-font-small)' }}>{formatDateRange()}</span>
              </div>
            </div>
          </div>
          ...
          <ScrollArea style={{ flex: 1, minHeight: 0 }}>
            <div style={{ padding: 'var(--op-space-large)' }}>
              {isLoading ? (
                <p style={{ color: 'var(--op-color-neutral-on-plus-max)', textAlign: 'center', padding: 'var(--op-space-x-large)' }}>
                  Loading leads...
                </p>
              ) : leads.length === 0 ? (
                <div style={{
                  ...
                  <p style={{ margin: 0, fontSize: 'var(--op-font-medium)', fontWeight: 600 }}>
                    No leads found for the selected date range
                  </p>
                  <p style={{ margin: 'var(--op-space-small) 0 0 0', fontSize: 'var(--op-font-small)' }}>
                  ...
                  {leads.map((lead) => (
                    <div key={lead.id}>
                      <div style={{
                        ...
                        {lead.visitor_name || lead.visitor_email || 'Anonymous'} · {new Date(lead.created_at).toLocaleDateString()}
                      </div>
                      <LeadSummary
                        data={lead.summary}
