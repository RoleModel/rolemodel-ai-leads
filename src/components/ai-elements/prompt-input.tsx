...
import { SentIcon, Loading03Icon, SquareIcon, Alert02Icon } from '@hugeicons-pro/core-stroke-standard';
...
export const PromptInput = ({
  ...
  const files = usingProvider ? providerFiles : items;
  ...
  const addFiles = useCallback(
    (incoming: File[] | FileList) => {
      if (usingProvider && controller) {
        controller.attachments.add(incoming);
        return;
      }
      addLocal(incoming);
    },
  ...
  const handleSubmit: FormEventHandler<HTMLFormElement> = (event) => {
    ...
    const text = usingProvider && controller
  ...
  const inner = (
    <>
      ...
      <form
        ...
        {...props}
      >
        <InputGroup direction="column" className="overflow-hidden">{children}</InputGroup>
      </form>
    </>
  );
...
export const PromptInputTextarea = ({
  ...
  return (
    <InputGroupTextarea
      id="prompt-input-message"
      className={cn("field-sizing-content full-width max-h-48 min-h-16", className)}
      name="message"
      ...
      aria-label={placeholder || "Enter your message"}
      {...props}
      {...controlledProps}
    />
  );
...
export const PromptInputSubmit = ({
  ...
  let Icon = <HugeiconsIcon icon={SentIcon} size={24} />;

  if (status === "submitted") {
    Icon = <HugeiconsIcon icon={Loading03Icon} size={24} />;
  } else if (status === "streaming") {
    Icon = <HugeiconsIcon icon={SquareIcon} size={24} />;
