...
const outputPath =
  args[1] || path.join(__dirname, '../public/images/halftone_swirl_data.json')
...
const viewBox = viewBoxMatch ? viewBoxMatch[1].split(' ').map(Number) : [0, 0, 2000, 1419]
...
const useRegex =
  /<use transform="translate\(([^,]+),\s*([^)]+)\)\s*scale\(([^)]+)\)(?:\s*rotate\([^)]+\))?"[^>]*fill="([^"]+)"/g
